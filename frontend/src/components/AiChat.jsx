...
function AiChat({ projectId, history = [], reloadTree }) {
    ...
    const handleChat = async () => {
        ...
        try {
            ...
            console.log(error)
            setMessages((prev) => [...prev, { role: "assistant", content: error.message || "AI request failed.", error: true }])
            return null
        }
        finally{
            setLoading(false)
    ...
    return (
        <div className='flex w-80 shrink-0 flex-col border-l border-white/[0.06] bg-[#0e0e12]/95 backdrop-blur-xl'>
            <div className='flex h-10 shrink-0 items-center justify-between border-b border-white/[0.06] px-3.5'>
                <div className='flex items-center gap-2'>
                    <div className='flex h-5 w-5 items-center justify-center rounded-md bg-gradient-to-br from-indigo-500 to-sky-500 text-white shadow-sm'>
                        <Sparkles size={11} />
                    </div>
                    <span className='text-xs font-semibold tracking-wide text-zinc-200'>Codexia AI</span>
                </div>
            ...
            <div className="border-t border-white/[0.06] p-3 bg-white/[0.01]">
                <div className='flex items-end gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] p-2.5 transition-all focus-within:border-sky-400/50 focus-within:bg-white/[0.05] focus-within:ring-2 focus-within:ring-sky-400/10'>
                    <textarea
                        ...
                        placeholder="Ask Codexia AI to write or fix code..."
                        rows={2}
                        className="flex-1 resize-none bg-transparent text-[13px] text-zinc-200 outline-none placeholder:text-zinc-500 [scrollbar-width:none]"
                    />
                    <motion.button
                        onClick={handleChat}
                        ...
                        className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-b from-sky-500 to-indigo-600 text-white shadow-[0_1px_0_rgba(255,255,255,0.2)_inset] transition-all hover:from-sky-400 hover:to-indigo-500 disabled:cursor-not-allowed disabled:opacity-35"
                    >
