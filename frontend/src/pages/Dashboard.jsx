...
import { Code2, Folder, Loader2, Menu, Plus, Sparkles, X } from 'lucide-react';
    ...
    if (!userData) {
        return (
            <div className='relative flex h-screen w-full items-center justify-center overflow-hidden bg-slate-50 px-4 transition-colors duration-300 dark:bg-[#07070c]'>
                ...
                <div className='pointer-events-none absolute -top-32 left-1/2 hidden h-[600px] w-[600px] -translate-x-1/2 rounded-full bg-indigo-500/[0.06] blur-[140px] dark:block' />

                <div className='relative w-full max-w-sm rounded-3xl border border-slate-200/80 bg-white/85 p-7 sm:p-9 text-center shadow-2xl shadow-slate-200/50 backdrop-blur-2xl dark:border-white/[0.08] dark:bg-white/[0.03] dark:shadow-black/60'>

                    <div className='mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 via-sky-500 to-indigo-600 shadow-[0_4px_20px_-4px_rgba(99,102,241,0.5)]'>
                        <Code2 size={26} className='text-white' strokeWidth={2.2} />
                    </div>
                    ...
                    <button
                        onClick={handleLogin}
                        disabled={loading}
                        className='flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white py-2.5 text-[13.5px] font-medium text-slate-800 shadow-sm transition-all duration-150 hover:bg-slate-50 hover:border-slate-300 disabled:opacity-70 dark:border-white/[0.08] dark:bg-white/[0.06] dark:text-white dark:hover:bg-white/[0.1]'>
                        <FcGoogle size={18} />
...
'
                        ...
                        <div className='mb-4 flex items-center justify-between'>
                            <h2 className='text-[16px] font-semibold tracking-tight text-slate-900 dark:text-white'>
                        ...
                        {loadingProjects ? (
                            ...
                            <div className='mb-8 flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300/80 bg-white/50 px-4 py-14 sm:py-16 text-center backdrop-blur-sm dark:border-white/[0.08] dark:bg-white/[0.015]'>
                                <div className='mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 dark:bg-white/[0.05] border border-slate-200/60 dark:border-white/[0.08]'>
                                    <Folder
                                        size={22}
                                ...
                                <h3 className='mb-1.5 text-[15.5px] font-semibold text-slate-900 dark:text-white'>
                                    {activeSession == "starred" ? "No starred projects" : "No projects yet"}
                                </h3>

                                <p className='mb-6 max-w-xs text-[13px] text-slate-500 dark:text-zinc-500 leading-relaxed'>
                                    {activeSession == "starred" ? "Star a project to pin it to your favorites." : "Create your first project to start coding with Codexia AI."}
                                </p>

                                <button
                                    onClick={() => setOpenModal(true)}
                                    className='inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-[13px] font-medium text-white shadow-sm hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 transition-all'
                                >
                                    <Plus size={15} />
                                    Create Project
                                </button>
