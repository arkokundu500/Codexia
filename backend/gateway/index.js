...
const allowedOrigins = [
    ...
    "http://localhost:5173",
    "http://localhost:5174",
    "https://d1vbsvfbkgp6gf.cloudfront.net"
].filter(Boolean)
...
// AI service uses http-proxy (not express-http-proxy) to support SSE streaming
const aiProxy = httpProxy.createProxyServer({
    target: process.env.AI_SERVICE || "http://localhost:8004"
})
...
aiProxy.on("proxyReq", (proxyReq, req) => {
    if (req.user?._id) {
...
app.use("/api/ai", protect, express.json(), (req, res) => {
    if (req.user?._id) {
        req.headers["x-user-id"] = String(req.user._id)
    }
    aiProxy.web(req, res, {
        target: process.env.AI_SERVICE || "http://localhost:8004"
    })
